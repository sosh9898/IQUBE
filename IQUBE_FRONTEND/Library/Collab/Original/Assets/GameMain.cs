...
public class GameMain : MonoBehaviour
{
    ...
    public void makedata()
    {
        ...
        ChatTest.Data B = new ChatTest.Data();

        B=A.revChat();
        int itemsize = A.revChat().size;
        // size

        for(int i=0; i < itemsize; i++)
        {

            ItemList2[i].Name = A.revChat().id; // 맵 이름
            ItemList2[i].User = A.revChat().id; // 유저 이름
            ItemList2[i].Mapsize = A.revChat().size; // 0 normal, 1 large
            ItemList2[i].Mode = A.revChat().mode; // 0 classic, 1 extend
